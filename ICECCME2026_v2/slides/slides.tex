  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.45\textwidth}
      ...
      \begin{itemize}\itemsep1pt
        ...
        \item Acceptance \emph{unchanged} vs.\ RTX4090: the model behaves
              the same, the \emph{stack} does not.
...
anecdote: ``acceptance is almost identical across the two devices --- 0.336
versus 0.335 at $k{=}4$. So the algorithm is behaving exactly as it did on the
desktop. What changed is the runtime economics: on this stack the draft model's
per-step cost relative to the target's verification cost is unfavourable, and
speculative decoding needs that ratio to be small.''\\[4pt]
...
\begin{frame}{Backup --- acceptance portability}
  ...
  \small Qwen3 acceptance is essentially identical on RTX4090 and
  RTX5090-laptop at every $k$, in both regimes.
  The algorithm behaves the same; the \alert{runtime economics} differ.
\end{frame}
\note{%
...
answer: acceptance is a model-level quantity and it is unchanged, so the
speculative loop is doing exactly what it did on the desktop. The loss is in
per-step execution cost, not in the algorithm.
}
