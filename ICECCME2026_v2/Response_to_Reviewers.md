...
Author Action: Added sample-level mean$\pm$std dispersion (computed across the real $n{=}1000$ evaluated samples per configuration), full TTFT/TPOT breakdowns, and Holm-Bonferroni-corrected one-sided paired $t$-tests with Cohen's $d$ effect sizes, comparing each fixed-$k$ configuration against its regime-matched baseline. We note explicitly that this reflects sample-level (not seed-to-seed) dispersion, since independent multi-seed repetition was not performed; this is named as a threat to validity and future work.
...
| R2-6 | Missing variance across independent runs/seeds. | Addressed with real sample-level mean$\pm$std reporting ($n{=}1000$ real samples per configuration, single seed) for all six fixed-$k$ runs. Explicitly distinguished from seed-to-seed variance in the text; independent multi-seed repetition of full runs was not performed and is named as a threat to validity for future work. | Sec. 05 (Sample-level dispersion subsection), Sec. 06 (Threats to validity) |
...
| Statistical transparency | Added real sample-level mean$\pm$std dispersion, Holm-Bonferroni-corrected paired $t$-tests, and Cohen's $d$ effect sizes, all computed directly from real per-sample measurements; explicitly distinguished from (not-performed) seed-to-seed variance. |
