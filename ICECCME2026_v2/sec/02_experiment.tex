...
sample-level summaries (mean$\pm$std) and one-sided paired $t$-tests with
Holm-Bonferroni correction and Cohen's $d$ effect sizes, comparing each
fixed-$k$ configuration against its regime-matched autoregressive baseline.
For repeated RTX4090 and RTX5090-laptop $k{=}4$ settings in both Qwen2.5 and
Qwen3, we also report seed-to-seed stability summaries across seeds
$\{42,123,999\}$ where the seed-42 primary result is available.
