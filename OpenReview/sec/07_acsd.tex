...
motivates a targeted extension:
...
ACSD is implemented in the current code and notebook pipeline (Phase 7/8),
including adaptive-$k$, rescue cooldown, delayed fallback, and periodic
checkpoint writes for long runs. Quantitative ACSD headline results are still
being finalized and are therefore reported separately from the fixed-policy
summary table in Section~\ref{sec:results}.
